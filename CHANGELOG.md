...
- **「检查更新」全程静默无反馈**：此前点击「检查更新」后，无论「已是最新版本」
  还是「检查失败」（GitHub 直连超时/断网），界面上都毫无动静——失败只写
  主进程日志，「无更新」事件干脆没有监听，用户体感就是自动更新坏了。现在
  用户主动检查的三种结果全部可见：
  - **已是最新** → 提示「已是最新版本（vX.Y.Z）」；
  - **检查失败** → 弹出原因与建议（检查网络/系统代理后重试），并提供
    「打开下载页」直达 Releases；
  - **下载失败** → 弹出原因并提供「打开下载页」。
  启动期的自动检查保持静默（失败不打扰），更新源发现链路实测正常
  （GitHub provider 正确解析 latest.yml 与 release notes；electron-updater
  走 Electron 网络栈、跟随系统代理，无法直连 GitHub 时开系统代理即可）。
...
- **并行多窗口**：一个窗口 = 一个连接 = 一个会话。**一 URL 一窗口**：重复连接同一地址只聚焦已有窗口，不同地址各自开窗，上限 **8 个**；菜单「⋯ → 新建连接窗口…」与命令面板随时加开 login 窗口。重启自动恢复最近 **5 个**会话窗口（含各窗口的位置/尺寸/最大化/缩放/置顶，按连接分别记忆）。托盘菜单列出全部会话（带未读前缀）点击聚焦，未读角标/徽章**跨窗口求和**，系统通知带连接名。多窗口纯策略（会话键/上限/恢复计划/未读聚合/最近活跃轮转/分区名/托盘标签）抽为 `session-policy.ts`，21 例单测。
- **每连接独立会话分区**：每个连接使用独立 `persist:dsh-<连接id>` 分区——cookie/storage 互不串扰，多账号并存、互不掉线（v0.7 的单 `persist:dsh` 升级）。**代价（接受并在此明示）**：每个连接首次打开都需要重新登录一次 DSH；这是按连接隔离与 A5 每连接代理的前提。
- **每连接代理（A5）**：login「已保存连接」条目新增 **⇄** 内联编辑器——代理地址（仅 `http/https/socks/socks4/socks5` scheme、必须显式端口、拒绝内嵌凭据/路径/查询串，明文 JSON 不存凭据）+ 绕过列表（逗号/分号/空白分隔，小写去重），「直连（清除代理）」一键清空。连接时仅对该连接自己的分区 `session.setProxy`，不影响其他窗口；已配置代理的连接在行上以「SOCKS host:port（绕过 n 条）」徽标展示。解析/校验/展示/转 Electron 入参抽为纯函数 `proxy.ts`，16 例单测；UI 冒烟覆盖编辑器展开/保存桥接归一化/回显/直连/取消全链路。
- **连接健康（A3）**：「服务器 ▾ → 连接健康…」（命令面板同入口）：对当前服务做一次就绪探测，展示响应时延、页面暴露的 DSH 版本、本机监听进程 PID。判定逻辑为纯函数 `health.ts`（17 例单测），探测执行在 `health-runtime.ts`。
- **全局热键扩到 5 个（C4）**：原 `Ctrl+Shift+D` 唤起之外新增——`Ctrl+Alt+N` 轮转到下一个会话窗口、`Ctrl+Alt+W` 关闭当前会话窗口、`Ctrl+Alt+R` 重启本应用启动的本地服务、`Ctrl+Alt+S` 停止全部本地服务；全部可在快捷键设置面板重绑/解绑（进动作白名单）。
- **关闭行为设置项**：「⋯」菜单 checkbox「关闭时收进托盘（否则关闭该会话窗口）」——默认**直接关闭该会话窗口**（多窗口下关一个不应整个藏起来），勾选后回到旧「关闭 = 藏进托盘」语义。退出询问、托盘兜底不受影响。

### 变更

- **结构重构：Session 抽取**——一个连接的完整生命周期（窗口事件、内容视图挂摘、查找栏、缩放、设置/命令面板互斥、重连轮询、未读、标题栏菜单、窗口级 IPC）抽为 `src/session.ts`；`main.ts` 从 2015 行减到 1140 行，只留应用级职责（会话注册表、托盘、全局热键、连接配置库、勿扰、主题、共享配置、更新、服务注册表、深链、引导）。行为不变，逐步抽取、每步全量测试。
- **修复潜伏 bug**：`login:pin-connection` 的 IPC handler 原本被错误注册在 `rename-connection` 回调内部（每次重命名会重复注册一份），现随 Session 抽取归位为独立注册。
- **效率五项**（测量存档见 [`docs/perf-baseline.md`](docs/perf-baseline.md)）：
  1. 共享配置轮询 5 s → `fs.watch` 为主通道 + 60 s 兜底；
  2. `shell-state.json` 改内存态 + 直写，去掉每次「读-改-写」（顺带消除多窗口写竞争）；
  3. 启动同步 I/O（迁移、desktopExe 写入、配置监听、更新检查）与自动连接移出首帧路径（实测先建窗口后排托盘，`ready-to-show` 快 ~16 ms）；
  4. 重连探测修掉「3 s 间隔 vs 5 s 超时」的重叠 bug（2.5 s 超时 + in-flight 去重）；
  5. 托盘菜单按状态签名缓存（原来每次 SPA 导航都重建整棵 `Menu`）。
- **包体瘦身（Windows）**：`electronLanguages: [en-US, zh-CN]` + 排除 `dist/**/*.map`——`locales/` 从 55 个/46.65 MB 到 2 个/1.09 MB，win-unpacked 349.76 → 304.27 MB（−13.0%），NSIS 安装包 95.43 → 87.50 MB（−8.3%）；portable 85.86 → 87.28 MB（+1.42 MB，原因未查明）。`electron-builder.yml` `releaseType` 进入正式 release。
- **文档**：新增 [`docs/perf-baseline.md`](docs/perf-baseline.md)——冷启动/稳态 I/O/包体的测量方法、原始数据与基线结论；README 同步多窗口、A3/A5/D2/C4、关闭行为与每连接分区说明。

### 诚实记录（已知代价与未做项）

- **冷启动 +13 ms**：0.8.1 中位 291 ms → 1.0.0 中位 304 ms（7+7 轮交替取中位，含测量方法与原始值）。效率收益在稳态 I/O 与包体，不在启动。
- **每连接首次需重新登录 DSH**：独立分区的必然代价（上文已明示）。
- **portable 体积反增 1.42 MB**：未查明，后续排查。
- **未做**（本轮明确排除）：统一错误处理与日志层（`main.ts` 尚有 3 处 `console.warn` 直写而非走诊断缓冲）、IPC 频道名契约收口、C2 quick-ask 浮窗、D1 safeStorage 凭据存储、D4 会话历史入口、三平台本机构建。
