...
- **Login 连接界面**：左侧官方鲸鱼 logo 品牌区，右侧三种连接方式——**本地嗅探**（探测本机已运行的 DSH Web，列出实例一键连接）/ **GUI 启动本地服务器**（spawn `dsh web` 并显示实时进度，自动在 npx 缓存里找 `dsh`）/ **云端服务器**（输入远程 URL，非回环地址弹确认）。另有**最近连接**列表（记忆最近 5 个地址，点击重连）。仅首次或配置失效时出现，有配置时直接连接零打扰。
...
npm run test:unit           # 仅 src（vitest：url / sniffer / protocol / shell-state）
